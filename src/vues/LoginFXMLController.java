...
import static sportifyj.SportifyJ.Userconnected;
...
public class LoginFXMLController implements Initializable {
    ...
    private void MdpOublie(MouseEvent event) {
        GotoFXML("ResetPasswordFXML", "Trouvez votre compte.", event);
    }
    ...
    private void connexion(ActionEvent event) throws IOException {
        ...
        if (Userconnected.getId() != 0) {
            ...
            if ("Admin".equals(Userconnected.getRole())) {
                GotoFXML("MainAdminFXML", "Bienvenue",event);
            } else {
                GotoFXML("MainClientFXML", "Bienvenue",event);
